...
const DocumentManagementUI: React.FC<Props> = ({
  ...
  const isChecklistLocked = userData?.productId !== "prod_SIo6C0oz646SIN";

  // Utility to check if doc exists in a list
  const isInList = (
    doc: { id: string | number },
    list: { id: string | number }[]
  ) => list.some((d) => String(d.id) === String(doc.id));

  // Check if document should be clickable based on selected filter
  const isDocumentClickable = (doc: GroupedDocument, columnType: string) => {
    // If checklist or detailed knowledge are locked, they're not clickable
    if (isChecklistLocked && (columnType === "CL" || columnType === "DK")) {
      return false;
    }

    if (selectedFilter === "ML") {
      // Only documents in relevantMLPDFList are clickable in all columns
      return isInList(doc, relevantMLPDFList);
    }
    ...
    switch (columnType) {
      case "ML":
        return isInList(doc, relevantMLPDFList);
      case "CL":
        return isInList(doc, relevantCLPDFList);
      case "DK":
        return isInList(doc, relevantDKPDFList);
      default:
        return true;
    }
  };
  ...
  const renderHeaderWithLock = (title: string, columnType: string) => {
    const isLocked =
      isChecklistLocked &&
      (columnType === "checklist" || columnType === "detailedKnowledge");
    ...
    if (isLocked) {
      return (
        <div className="relative">
          ...
          <div className="p-2 sm:p-4 font-semibold text-gray-900 text-center border-r border-gray-200 relative text-xs sm:text-base">
            {title}
            {/* Subscribe button */}
            <button
              onClick={() => route.push("/payment/price")}
              className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium px-2 py-1 sm:px-3 sm:py-1 rounded-md transition-colors"
            >
              Subscribe
            </button>
          </div>
    ...
    return (
      <div className="p-2 sm:p-4 font-semibold text-gray-900 text-center border-r border-gray-200 text-xs sm:text-base">
        {title}
      </div>
    );
  };
        ...
        {documents.length > 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col">
            ...
            >
              ...
              <div className="p-2 sm:p-3 lg:p-4 font-semibold text-gray-900 text-center text-xs sm:text-sm lg:text-base">
