...
const SearchInputBubbleComponent: React.FC<SearchInputProps> = ({
  input,
  setInput,
  searchHandler,
  className,
    ...
    <div className={`${className}`}>
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 px-2 cursor-pointer text-xs text-gray-600 hover:text-gray-800 transition-colors border-r"
        >
          {selectedCategory}
          ...
          <div className="absolute top-full left-0 mt-1 w-32 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
            {categories.map((category: string) => (
              <button
                ...
                className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-gray-100 cursor-pointer first:rounded-t-lg last:rounded-b-lg"
              >
                {category}
              </button>
            ))}
      ...
      <textarea
        ...
        placeholder={`${
          selectedCategory !== "Question" ? "E.g., 430" : "E.g., Message"
        }`}
        value={input}
        onChange={(e) => {
...
export default SearchInputBubbleComponent;
